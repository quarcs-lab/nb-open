---
title: "Exploring labor productivity differences across countries and over time"
...
```{r}
log_ky_raw <- prepare_quantile_trend_graph(dat, "year", c(0.05, 0.25, 0.5, 0.75, 0.95), "log_ky_raw", points = FALSE)$plot
```
...
ggsave("figs/quintiles_all_log_ky_raw.pdf", width = 6, height = 4)
```   
...
```{r}
log_h_raw <- prepare_quantile_trend_graph(dat, "year", c(0.05, 0.25, 0.5, 0.75, 0.95), "log_h_raw", points = FALSE)$plot
```
...
ggsave("figs/quintiles_all_log_h_raw.pdf", width = 6, height = 4)
```  
...
```{r}
log_tfp_raw <- prepare_quantile_trend_graph(dat, "year", c(0.05, 0.25, 0.5, 0.75, 0.95), "log_tfp_raw", points = FALSE)$plot
```
...
ggsave("figs/quintiles_all_log_tfp_raw.pdf", width = 6, height = 4)
```   
